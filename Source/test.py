...
def test(Test,f,Train):
  ...
  for train_features, train_labels in Test:
      X,Y = utils.apply_transform(train_features,train_labels,state=1)
      i=0
      for x in X:
        x = x.reshape(1,1,128,313)
        h = f(x)
        H.append(h)
        y_pred.append(utils.anomaly_score(h.detach().numpy(),v_mean.detach().numpy(),v_cov))
